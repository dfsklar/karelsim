...
jsim.isUserFunction = function(node) {
	...
	this.validateObject(node.name, "jsim.isUserFunction", "name");
	this.validateObject(node.name.name, "jsim.isUserFunction", "name.name");
...
jsim.ges = function(node, arrExpr, curName, curNum, subNum, lineNumber, sb) {
	...
	if ( typ === "NullLiteral" ) {
		...
		//QQQ jsim.validateObject  (node.operator,       "jsim.ges:AssignmentExpression", "operator", ["="]);  // QQQ handled in gss???
...
jsim.gssReturnStatement = function(sb, node, curName, curNum) {
	...
	arrExpr = [];
	this.ges(node.value, arrExpr, curName, curNum, 0, line, sb);
	expr = arrExpr.join(" ");
...
jsim.gssVariableStatement = function(sb, node, curName, curNum) {
	...
	for ( ii = 0, len = decls.length; ii < len; ii += 1 ) {
	    ...
	    arrExpr = [];
	    this.ges(decls[ii].value, arrExpr, curName, curNum, 0, line, sb);
...
jsim.gssIfStatement = function(sb, node, curName, curNum) {
	...
	arrExpr = [];
	this.ges(cond, arrExpr, curName, curNum, 0, line, sb);
	expr = arrExpr.join(" ");
